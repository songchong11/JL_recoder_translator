...
struct __mic_effect {
    ...
    struct __stream_entry 	*mic_enhance_hdl;
...
static void mic_effect_destroy(struct __mic_effect **hdl)
{
    ...
    if (effect->mic_enhance_hdl) {
        stream_entry_close(&effect->mic_enhance_hdl);
    }
    if (effect->drc) {
        audio_drc_close(effect->drc);
        free(effect->drc);
    }
...
#endif
static int mic_effect_enhance_callback(void *priv, struct audio_data_frame *in)
{

    struct __mic_effect *effect = (struct __mic_effect *)priv;
    s32 tdat[200];//与每次运行的点数对应
    s16 *data = in->data;
    u32 len = in->data_len;
    u8 i;
    for (i = 0; i < len / 2; i++) {
    ...
    audio_drc_run(effect->drc, tdat, len * 2); //受TCFG_DRC_ENABLE控制
    for (i = 0; i < len / 2; i++) {
        data[i] = data_sat_s16(tdat[i]);
    }
    return len;
}
static struct drc_ch high_bass_drc = {0};
int mic_drc_get_filter_info(struct audio_drc *drc, struct audio_drc_filter_info *info)
{

    //    int th = high_bass_th;//-60 ~0db
    int th = -6;//-60 ~0db //限幅起输出的最大幅度，尽量保证混响叠加后不失真
    ...
    high_bass_drc._p.limiter[0].releasetime = 500;
...
bool mic_effect_start(void)
{
    ...
    if (effect->parm.effect_config & BIT(MIC_EFFECT_CONFIG_DIGIT_ENHANCE)) {
        effect->mic_enhance_hdl = stream_entry_open(effect, mic_effect_enhance_callback, 0);
        effect->drc = malloc(sizeof(struct audio_drc));
        if (effect->drc) {
            struct audio_drc_param drc_param = {0};
            drc_param.channels = ch_num;
            drc_param.cb = mic_drc_get_filter_info;
            drc_param.drc_name = 0;
            audio_drc_open(effect->drc, &drc_param);
            audio_drc_set_samplerate(effect->drc, effect->parm.sample_rate);
            audio_drc_set_output_handle(effect->drc, NULL, NULL);
            audio_drc_set_32bit_mode(effect->drc, 1);
            audio_drc_start(effect->drc);
        }
...
// 数据流串联
    ...
    if (effect->mic_enhance_hdl) {
        entries[entry_cnt++] = &effect->mic_enhance_hdl->entry;
    }
    if (effect->p_howling_hdl) {
